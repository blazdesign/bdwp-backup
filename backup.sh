...
DB_BACKUP_SIZE=`du -k ~/bdwp-backup/tmp/$DATABASE_FILENAME | cut -f1`
DB_BACKUP_END=$(date +%s)

echo -e "DB Export Time: $(($DB_BACKUP_END - $DB_BACKUP_START))s\n"
echo -e "DB Backup Size: $(($DB_BACKUP_SIZE))kb\n"
...
FILE_BACKUP_SIZE=`du -k ~/bdwp-backup/tmp/$FILE_BACKUP_FILENAME | cut -f1`
FILE_BACKUP_END=$(date +%s)

echo -e "File Backup Time: $(($FILE_BACKUP_END - $FILE_BACKUP_START))s\n"
echo -e "File Backup Size: $(($FILE_BACKUP_SIZE))kb\n"
...
#Generate email 
./make-email.sh $DROPBOX_DIR $DIR $DATABASE_FILENAME $FILE_BACKUP_FILENAME $DATABASE_BACKUP_SIZE $FILE_BACKUP_SIZE > ~/bdwp-backup/tmp/output.html
